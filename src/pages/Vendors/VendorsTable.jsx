...
import { deleteVendor, fetchVendors } from "../../Store/Vendors/vendors";
import { useDispatch, useSelector } from "react-redux";
import { Vendortoggle } from "../../Store/Vendors/vendorToggle";
...
export default function VendorsTable({
  ...
  const status = useSelector((state) => state.vendorToggle);
  ...
  const filteredVendors = useMemo(() => {
    return vendors.filter((vendor) => {
      const matchesSearch =
        ...
        vendor.ArName?.toLowerCase().includes(search) ||
        vendor.place?.name.toLowerCase().includes(search) ||
        vendor.place?.ArabName.toLowerCase().includes(search) ||
        vendor.country?.name.toLowerCase().includes(search) ||
        vendor.country?.EngName.toLowerCase().includes(search) ||
      ...
      const matchesPlace = placeFilter
        ? vendor.place?.id === Number(placeFilter)
        : true;

      return matchesSearch && matchesStatus && matchesPlace;
    });
  }, [vendors, placeFilter, statusFilter, search]);
  ...
  const pageData = filteredVendors.slice(startIndex, endIndex);
  ...
  const confirmDelete = () => {
    if (vendorToDelete) {
      dispatch(deleteVendor(vendorToDelete));
    }
    setModal(false);
  };
  const handleToggle = (vendorId, currentStatus) => {
    dispatch(Vendortoggle({ vendorId, currentStatus: status }));
  };
  ...
  return (
    <>
      <div className="relative overflow-x-auto">
        <div className="overflow-auto w-[1800px] max-h-[600px]">
          <table className="w-full text-left overflow-y-auto">
            <thead>
              <tr className="border-b border-gray-300">
                ...
                <th className="p-2">Doctor Title</th>
                <th className="p-2">Doctor Specialization</th>
                <th className="p-2">Description</th>
                <th className="p-2">Address</th>
                <th className="p-2">Phone1</th>
                <th className="p-2">Phone2</th>
                <th className="p-2">Phone3</th>
                ...
                pageData?.map((offer, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-2">
                        {offer.name} <br /> ({offer.ArName}){" "}
                      </td>
                      <td className="p-2">{offer.email}</td>
                      <td className="p-2">
                        {offer.place?.name} <br /> ({offer.place?.ArabName})
                      </td>
                      <td className="p-2">
                        {offer.doctor_title?.title || "---"} <br /> (
                        {offer.doctor_title?.ArabTitle || "---"})
                      </td>
                      <td className="p-2">
                        {offer.doctor_specialization?.specialization || "---"}{" "}
                        <br /> (
                        {offer.doctor_specialization?.ArabSpecialization ||
                          "---"}
                        )
                      </td>
                      <td className="p-2 w-44">
                        {offer.description} <br /> ({offer.ArDescription})
                      </td>
                      <td className="p-2 w-40">
                        {offer.address} <br /> ({offer.ArAddress})
                      </td>
                      <td className="p-2 w-24">
                        {offer?.phone1 ? offer?.phone1 : "----"}
                      </td>
                      <td className="p-2">
                        {offer?.phone2 ? offer?.phone2 : "----"}
                      </td>
                      <td className="p-2">
                        {offer?.phone3 ? offer?.phone3 : "----"}
                      </td>
                      <td className="p-2 text-blue-500">
                        <Link to={offer.location}>
                          <FiMapPin />
                        </Link>
                      </td>
                      <td className="p-2">
                        {offer.country?.EngName} <br /> ({offer.country?.name})
                      </td>
                      <td className="p-2">
                        {offer.city?.name} <br /> ({offer.city?.ArabName})
                      </td>
                      <td className="p-2 w-20">
                        {offer.area?.EnArea} <br /> ({offer.area?.ArArea})
                      </td>
                      <td className="p-2">
                        <label className="inline-flex items-center cursor-pointer">
                          <input
                            ...
                            value=""
                            onChange={() =>
                              handleToggle(offer.id, offer.status)
                            }
                      ...
                      <td className="p-2 flex justify-between mt-[10px]">
                        <button
