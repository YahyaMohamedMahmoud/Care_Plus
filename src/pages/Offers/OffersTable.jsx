...
import { deleteOffer, fetchOffers } from "../../Store/Offers/offers";
// import { Link } from "react-router-dom";
import { toggleOfferStatus } from "../../Store/Offers/statustoggle";
import { AiOutlineArrowLeft, AiOutlineArrowRight } from "react-icons/ai";

export default function OffersTable({ setIsModalOpen , placeFilter , statusFilter , search , setSelectedOffer}) {
  ...
  const status = useSelector((state) => state.statustoogle);
  ...
  const filteredOffers = useMemo(() => {
    return offers?.filter((offer) => {
      const matchesSearch =
        ...
        offer.ArName?.toLowerCase().includes(search) ||
        offer.description?.toLowerCase().includes(search) ||
        offer.ArDescription?.toLowerCase().includes(search) ||
        offer.address?.toLowerCase().includes(search) ||
        offer.vendor?.name.toLowerCase().includes(search) ||
        offer.vendor?.ArName.toLowerCase().includes(search) ||
        offer.discount?.toLowerCase().includes(search);
  
      const matchesStatus = statusFilter ? offer.status === statusFilter : true;
      const matchesPlace = placeFilter ? offer.vendor?.id === Number(placeFilter) : true;
  
      return matchesSearch && matchesStatus && matchesPlace;
    });
  }, [offers, placeFilter, statusFilter, search]);
  ...
  const handleToggle = (offerId , currentStatus) => {
    dispatch(toggleOfferStatus({ offerId, currentStatus :status}));
  };
  ...
  const confirmDelete = () => {
    if (offerToDelete) {
      dispatch(deleteOffer(offerToDelete)); 
    }
    setModal(false); 
  };
  ...
  const currentPageData = filteredOffers?.slice(startIndex, startIndex + itemsPerPage);
                ...
                currentPageData?.length <= 0 ? (
                  <tr className="text-center">
                    ...
                    <td colSpan="12" className="py-8 text-gray-700">
                      <div>
                        <h2 className="text-2xl font-semibold">No offers available at the moment.</h2>
                      </div>
                    </td>
                  </tr>
                ) : 
                currentPageData?.map((offer ,index) => {
                  const globalIndex = startIndex + index + 1;
               return(
                <tr key={offer.id} className="border-b border-gray-300">
                <td className="p-2">
                  <input type="checkbox" />
                </td>
                <td className="p-2">{globalIndex}</td>
                <td className="p-2">
                  <img src={offer.photo} alt={offer.name} loading="lazy" className="w-10 h-10 rounded-full" />
                </td>
                <td className="p-2">{offer.name}<br/> ({offer.ArName}) </td>
                <td className="p-2">{offer.discount?.slice(0,2)}</td>
                <td className="p-2">{offer.vendor?.name} <br/> ({offer.vendor?.ArName}) </td>
                {/* <td className="p-2 w-44">{offer.description}</td>
                <td className="p-2 w-48">{offer.address}</td> */}
               <td className="p-2 w-44">{offer.description} <br/> ({offer.ArDescription})</td>
                <td className="p-2">{`${offer.created_days} days left`}</td>
                <td className="p-2">
                ...
                <td className="p-2 flex justify-center mt-[10px]">
                  ...
                  <button className="delete" onClick={() => handleDeleteOffer(offer.id)}>
                    <svg
                      id="Group_1755"
